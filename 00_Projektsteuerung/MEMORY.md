...
- **Startbildschirm komplett entfernt (v1.61.0, 2026-07-13)**: auf Nachfrage ("macht die Startseite nach all den Änderungen noch Sinn?") ehrliche Einschätzung gegeben — seit v1.59.0 duplizierte der Startbildschirm exakt das, was das immer sichtbare `#plan-select`-Dropdown schon konnte (Plan wechseln/anlegen). Nutzer-Entscheidung: ganz auflösen statt nur die doppelten Buttons zu entfernen. Plan-Verwaltung (⎘/✏/🗑), Übungs-Bibliothek und Physio-Übungen sind jetzt im Profil-Bereich ("Training"); der Physio-Fällig-Banner wurde in die normale Kartenansicht verschoben, damit er nicht verschwindet. Details siehe `PROJEKTLOG.md` v1.61.0 und `CLAUDE.md` ("Home screen removed").
- **CSV-Export der Sätze erledigt (v1.60.0, 2026-07-13)**: nach Brainstorm (Satz-Ebene, Session-Ebene, PR-Historie, Wochenvolumen-Rohtabelle als Optionen) hat der Nutzer sich für den schlanken Umfang entschieden: nur eine Sätze-CSV, keine zweite Sessions-Datei. Reines CSV statt `.xlsx` (keine neue Dependency), Semikolon-Trenner + Komma-Dezimalzahlen + UTF-8-BOM fürs deutsche Excel. Neuer Button "📊 CSV" im Profil, Bereich "Daten". Details siehe `PROJEKTLOG.md` v1.60.0.
  - **Offen geblieben, bewusst nicht Teil von v1.60.0**: Session-Ebene (Wellbeing/Notiz/Gesamtvolumen), PR-Historie über Zeit, Wochenvolumen-Rohtabelle als CSV — nur bei explizitem späteren Wunsch nachziehen, nicht von selbst ergänzen.
