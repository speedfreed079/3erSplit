...
- **Aufwärmen-Inhalt (`WARMUP` in index.html) soll überdacht/vereinfacht werden** (Notiz vom 2026-07-10, Recherche liegt seit 2026-07-11 vor): Nutzer möchte den Umfang deutlich reduzieren — im Kern nur noch **dynamisches Dehnen** plus **Cardio-Geräte-Wahl** (Crosstrainer/Fahrrad/Rudern statt nur Rudern), `activation`-Bandübungen und `rampSets` standen infrage. Der vorbereitete Recherche-Prompt (`01_Recherchen/03_Aufwaermen/Recherche-Prompt-Aufwaermen-Ueberarbeitung.md`) wurde vom Nutzer **versehentlich zweimal** ausgeführt — zwei Ergebnisse liegen vor (`Optimierung des Krafttraining-Aufwärmens.md`, `Wissenschaftliches Aufwärmen im Krafttraining.md`), die sich in zwei Punkten widersprechen: (a) Cardio-Gerät fix pro Tagestyp zuordnen vs. freie Wahl anbieten, (b) Band-Aktivierung/Rampenschema komplett behalten vs. differenziert kürzen (Glute-Aktivierung am Beintag streichen, Rampenschema auf 3 Stufen vom Arbeitsgewicht statt 4 Stufen vom 1RM vereinfachen). Noch keine Entscheidung getroffen, welcher Synthese gefolgt wird — nächster Schritt ist eine Rückfrage an den Nutzer, bevor `warmupSectionHTML()`/`WARMUP` geändert wird.
