...
- **Aufwärmen-Inhalt (`WARMUP` in index.html) soll überdacht/vereinfacht werden** (Notiz vom 2026-07-10): Nutzer möchte den Umfang deutlich reduzieren — im Kern nur noch **dynamisches Dehnen** (die bestehenden `mobilization`-Einträge treffen das schon in etwa) **plus Cardio-Geräte-Optionen** (Crosstrainer, Fahrrad, Rudern zur Auswahl, nicht nur Rudern wie aktuell in `general` hart hinterlegt). Die bestehenden `activation`-Einträge (Band-Übungen wie Face Pulls/Pull-Aparts) und `rampSets` (prozentuale Aufwärmsätze) stehen damit infrage — noch nicht final entschieden, was davon wegfällt. **Nutzer möchte dafür noch recherchieren** (eigene Gemini-Deep-Research-Session, gleiches Muster wie bisher — noch kein Prompt vorbereitet) bevor Änderungen umgesetzt werden. Betrifft `warmupSectionHTML()`/`WARMUP` für alle Tagestypen (push/pull/legs), nicht nur einen einzelnen Plan.
...
- **Damit ist die gesamte am 2026-07-10 erstellte UI/UX- und Feature-Wunschliste (Phase 1-4 + "Rest der Wunschliste") bis auf den KI-Dialog abgearbeitet.** Verbleibende offene Threads insgesamt: KI-Dialog (siehe oben), Calisthenics-Übungsdatenbank (wartet auf Nutzer-Recherche, Prompt liegt bereit), Admin-Panel-Funktionalität (wartet auf Backend-Entscheidung, siehe CLAUDE.md).
