...
- **CSV/Excel-Export für Datenanalyse — Idee, noch nicht umgesetzt (2026-07-13)**: unabhängig vom bestehenden JSON-Backup (Export/Import, Firestore-Sync, Google-Drive-Backup — die alle die App selbst sichern) ein zusätzlicher Export für Analyse in Excel/Sheets, gedacht für einen datenaffinen Nutzer, der selbst Pivot-Tabellen/Trendlinien bauen will. Gebraucht/besprochen:
  - **Satz-Ebene (long format, das Herzstück)**: eine Zeile pro Satz — Datum, Uhrzeit, Plan, Tag, Übung, Muskelgruppe, Gym, Satz-Nr, Gewicht, Wdh., e1RM (Epley), Könnte-mehr, Aufwärmsatz. Long statt wide, damit Excel-Pivots direkt funktionieren.
  - **Session-Ebene**: eine Zeile pro Trainingseinheit — Datum, Plan, Tag, Gym, Wellbeing, Notiz, Satzzahl, Gesamtvolumen.
  - **PR-Historie statt nur aktueller Bestwert**: `computePersonalRecords()` liefert nur den aktuellen Bestwert je Übung; für Analyse interessanter wäre eine Zeitreihe, *wann* welcher PR aufgestellt wurde.
  - **Wochenvolumen-Rohtabelle**: Woche × Muskelgruppe × Satzzahl als Tabelle statt nur als Chart im Tagebuch.
  - **Konsistenz-Rohdaten**: Liste der Trainingstage, für eigene Kalender-Heatmaps.
  - **Format-Entscheidungen, offen**: reines CSV (Semikolon statt Komma als Trennzeichen für deutsches Excel, UTF-8 mit BOM wegen Umlauten) vs. echtes `.xlsx` mit mehreren Reitern (bräuchte SheetJS o.ä. als einzige neue Dependency — Bruch mit der Zero-Dependency-Philosophie). Empfehlung aus dem Brainstorm: erstmal CSV (Sätze + Sessions als zwei Downloads), PR-Historie/Wochenvolumen als CSV später nachziehen.
  - **Nächster Schritt**: Umfang/Format mit Nutzer festlegen, dann als neues Profil-Feature ("📊 Als CSV exportieren") umsetzen — noch nicht begonnen.
- **5 Nutzer-Funde vom 2026-07-12 erledigt (v1.55.0)**: Tagebuch-Reiter-Reihenfolge getauscht, falsche "Heute dran"-Empfehlung nach rückdatiertem Abschluss behoben (Verdacht aus der Notiz bestätigt — `finishSession()` überschrieb `lastCompletedDay`/`-Date` bedingungslos), zu schmales KG-Feld verbreitert, Wortumbruch im Trainingsmodus durch neue gestapelte Titel/Button-Anordnung behoben, Stift-Button-Überlauf in der Kartenansicht per `min-width:0`-Fix behoben. Details siehe `PROJEKTLOG.md` v1.55.0.
...
- **Standard-Schriftgröße bleibt `"normal"` — endgültig entschieden, NICHT erneut fragen.** War von 2026-07-08 bis 2026-07-12 mehrfach als offene Frage notiert (Nutzer hatte einmal erwähnt, die größte Stufe `xgross` beim Ausprobieren angenehm zu finden) und wurde vom Nutzer im Chat wiederholt bestätigt: Default bleibt `"normal"`. Diese Zeile existiert nur noch, damit die Frage nicht in einer künftigen Session aus alten Notizen heraus nochmal aufkommt.
